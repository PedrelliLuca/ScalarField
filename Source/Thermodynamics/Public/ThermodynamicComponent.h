...
private:
    void _setHeatCapacity(double heatCapacity) { _heatCapacity = FMath::Clamp(heatCapacity, 1., TNumericLimits<double>::Max()); }
    ...
    /**
     * The following two functions specifically mention "Thermodynamics" in their names since SetThermodynamicCollision(), the function that sets up the
     * _thermodynamicCollisionC, requires the input component to be a "HeatExchanger". A "HeatExchanger" is a "Thermodynamics" object channel that can only
     * overlap with object that are also "Thermodynamics", so we have the warranty here that overlappedComponent is in the "Thermodynamics" channel.
     */
    UFUNCTION()
    void _onSimpleBeginOverlap(UPrimitiveComponent* overlappedComponent, AActor* otherActor, UPrimitiveComponent* otherComp, int32 otherBodyIndex,
    ...
    /**
     * Thermodynamic components whose simple collision is colliding with this component's simple collision. We don't know for sure if we can exchange heat with
     * them though, we need to check the relation between their most complex collisions. That occurs at each tick.
     */
    TSet<TWeakObjectPtr<UThermodynamicComponent>> _possibleHeatExchangers{};

    bool _bCollisionChangedSinceLastTick = false;
    bool _bHasNeverTicked = true;
};
