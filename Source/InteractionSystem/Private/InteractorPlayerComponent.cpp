...
bool UInteractorPlayerComponent::PerformInteractionCheck() {
	...
	if (FMath::IsNearlyZero(_getInteractableBeingInteracted()->GetInteractionTime())) {
		...
		GetWorld()->GetTimerManager().SetTimer(_interactionTimerHandle, this, &UInteractorPlayerComponent::_interact, _getInteractableBeingInteracted()->GetInteractionTime()); 
	}
