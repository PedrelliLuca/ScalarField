...
void UEnvironmentGridWorldSubsystem::_spawnCellAtCoordinates(
    const FCellCoordinates coordinates, const FVector2D gridCenter, const double side, const double z, const FVector diagonalCorrection) {
    // Cell spawn
...
void UEnvironmentGridWorldSubsystem::_onCellLeft(FCellCoordinates cellLeft) {
    ...
    Algo::CopyIf(neighborsOfLeft, uncommonNeighbors,
        [&overlappedAndNeighbors](const FCellCoordinates& neighborOfLeft) { return overlappedAndNeighbors.Find(neighborOfLeft) == nullptr; });
