...
void UCastAttachedActorSkill::_skillCast() {
    ...
    TObjectPtr<USpringArmComponent> const spawnSpringArm = NewObject<USpringArmComponent>(target.Get(), TEXT("SkillSpringArm"));
    ...
    auto const location = _actorToTarget.GetLocation();
    ...
    auto const relativeRotation = (-location).Rotation();
    ...
    auto const spawnedEntityWorldTransform = _actorToTarget * target->GetTransform();
    auto const spawnActor = GetWorld()->SpawnActorDeferred<AActor>(_actorClass, spawnedEntityWorldTransform);
    ...
    TWeakInterfacePtr<ISkillSpawnedEntity> const skillSpawnedEntity = Cast<ISkillSpawnedEntity>(spawnActor);
...
void UCastAttachedActorSkill::_skillChannelingTick(float const deltaTime) {
