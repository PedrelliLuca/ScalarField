...
TSet<EFaction> UFactionComponent::GetFactionsWithRelationship(const ERelationship queryRelationship) const {
	const auto predicate = [queryRelationship](const TPair<EFaction, ERelationship>& factionToRelationship) {
		return factionToRelationship.Value == queryRelationship;
	};
	
	TSet<EFaction> factionsWithRelationship;
	Algo::TransformIf(_relationships, factionsWithRelationship, predicate, [](const TPair<EFaction, ERelationship>& factionToRelationship) {
		return factionToRelationship.Key;
	});
